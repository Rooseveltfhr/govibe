...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <a href="{{ route('evenements.index') }}" class="text-sm text-msn-sea-700 hover:underline">&larr; Événements</a>

        <div class="mt-3 flex flex-wrap items-center gap-3">
            <h1 class="text-3xl font-bold text-msn-sea-900 sm:text-4xl">{{ $event->title }}</h1>
        ...
        @if ($event->location)
            <p class="mt-1 text-sm text-msn-sea-700">{{ $event->location }}</p>
        @endif

        <p class="mt-6 whitespace-pre-line text-msn-sea-700">{{ $event->description }}</p>
    </div>
@endsection
