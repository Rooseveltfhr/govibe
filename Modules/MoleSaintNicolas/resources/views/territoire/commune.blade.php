...
@section('content')
    <div class="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
        <a href="{{ route('territoire.index') }}" class="text-sm text-msn-sand-200 hover:underline">&larr; Territoire</a>

        <div class="mt-3 flex items-center gap-3">
            <h1 class="text-3xl font-bold text-msn-sand-100 sm:text-4xl">Commune de {{ $commune->name }}</h1>
            <x-content-status-badge :status="$commune->content_status" />
        </div>

        <p class="mt-4 max-w-2xl text-msn-sand-200">{{ $commune->description ?: '[Information à compléter]' }}</p>

        @if ($commune->population)
            <p class="mt-2 text-sm text-msn-sand-200">
                Population : {{ number_format($commune->population, 0, ',', ' ') }}
                @if ($commune->population_year) ({{ $commune->population_year }}) @endif
            </p>
        @endif

        <h2 class="mt-10 text-xl font-bold text-msn-sand-100">Sections communales</h2>
        <div class="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <a href="{{ route('territoire.section', [$commune->slug, $section->slug]) }}"
                   class="block rounded-2xl border border-msn-sea-700 bg-msn-sea-900 p-6 shadow-sm transition hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <h3 class="font-semibold text-msn-sand-100">{{ $section->name }}</h3>
            ...
            @empty
                <p class="text-msn-sand-200">[Information à compléter — aucune section communale enregistrée]</p>
            @endforelse
        </div>
    </div>
@endsection
