...
@section('content')
    <div class="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
        <a href="{{ route('territoire.index') }}" class="text-sm text-msn-ink-700 hover:underline">&larr; Territoire</a>

        <div class="mt-3 flex items-center gap-3">
            <h1 class="text-3xl font-bold text-msn-ink-900 sm:text-4xl">Commune de {{ $commune->name }}</h1>
        ...
        <h2 class="mt-10 text-xl font-bold text-msn-ink-900">Sections communales</h2>
        <div class="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <a href="{{ route('territoire.section', [$commune->slug, $section->slug]) }}"
                   class="block rounded-2xl border border-msn-sand-200 bg-white p-6 shadow-sm transition hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <h3 class="font-semibold text-msn-ink-900">{{ $section->name }}</h3>
            ...
            @empty
                <p class="text-msn-ink-700">[Information à compléter — aucune section communale enregistrée]</p>
            @endforelse
        </div>
    </div>
@endsection
