...
@section('content')
    <div class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold text-msn-ink-900 sm:text-4xl">Histoire de Môle-Saint-Nicolas</h1>
        ...
        <div class="mt-12 space-y-16">
            @forelse ($periods as $period)
                <section class="border-l-4 border-msn-terracotta-500 pl-6">
                    <div class="flex flex-wrap items-center gap-3">
                        <h2 class="text-2xl font-bold text-msn-ink-900">{{ $period->name }}</h2>
                        <span class="text-sm font-semibold text-msn-ink-700">
                    ...
                    <p class="mt-3 max-w-2xl text-msn-ink-700">{{ $period->description ?: '[Information à compléter]' }}</p>
                        ...
                        <ul class="mt-6 space-y-4">
                            @foreach ($period->events as $event)
                                <li class="rounded-xl bg-white p-4 shadow-sm">
                                    ...
                                    <p class="mt-1 font-semibold text-msn-ink-900">{{ $event->title }}</p>
                                    <p class="mt-1 text-sm text-msn-ink-700">{{ $event->description ?: '[Information à compléter]' }}</p>
                                </li>
                            @endforeach
                        </ul>
                    @endif
                    ...
                    @if ($period->figures->isNotEmpty())
                        <div class="mt-6">
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-msn-ink-700">Personnages</h3>
                            <ul class="mt-2 flex flex-wrap gap-2">
                                @foreach ($period->figures as $figure)
                                    <li class="rounded-full bg-msn-sand-200 px-3 py-1 text-sm text-msn-ink-900">{{ $figure->name }}</li>
                                @endforeach
                            </ul>
                        </div>
                    @endif
            ...
            @empty
                <p class="text-msn-ink-700">[Information à compléter — aucune période historique enregistrée]</p>
            @endforelse
        </div>
    </div>
@endsection
