...
@section('title', 'Événements — Administration')

@section('content')
    <div class="mx-auto max-w-5xl px-4 py-10">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-msn-sea-900">Événements</h1>
                ...
                <tbody class="divide-y divide-msn-sand-200">
                    @foreach ($events as $event)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-msn-sea-700">{{ $event->starts_at->format('d/m/Y H:i') }}</td>
                            <td class="px-4 py-3"><x-content-status-badge :status="$event->content_status" /></td>
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('admin.evenements.edit', $event) }}" class="text-msn-sea-700 hover:underline">Modifier</a>
                                <form method="POST" action="{{ route('admin.evenements.destroy', $event) }}" class="inline" onsubmit="return confirm('Supprimer cet événement ?')">
                                    @csrf @method('DELETE')
