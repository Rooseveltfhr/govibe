...
@section('content')
    <div class="mx-auto max-w-2xl px-4 py-10">
        <h1 class="text-2xl font-bold text-msn-sea-900">
            {{ $event->exists ? "Modifier {$event->title}" : 'Nouvel événement historique' }}
        </h1>
        ...
        <form method="POST"
              action="{{ $event->exists ? route('admin.histoire.events.update', $event) : route('admin.histoire.events.store') }}"
            ...
            <div>
                ...
                <select name="historical_period_id" class="mt-1 block w-full rounded-lg border-gray-300">
                    ...
                    @foreach ($periods as $period)
                        <option value="{{ $period->id }}" @selected(old('historical_period_id', $event->historical_period_id) == $period->id)>
                            {{ $period->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-msn-sea-900">Date précise (si connue)</label>
                    <input type="date" name="happened_on" value="{{ old('happened_on', optional($event->happened_on)->format('Y-m-d')) }}"
                           class="mt-1 block w-full rounded-lg border-gray-300">
                </div>
                <div>
                    <label class="block text-sm font-medium text-msn-sea-900">Année approximative (sinon)</label>
                    <input type="number" name="circa_year" value="{{ old('circa_year', $event->circa_year) }}"
                           class="mt-1 block w-full rounded-lg border-gray-300">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="mt-1 block w-full rounded-lg border-gray-300">{{ old('description', $event->description) }}</textarea>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.histoire.events.index') }}" class="rounded-lg border border-msn-sea-500 px-5 py-2.5 font-semibold text-msn-sea-900">
                    Annuler
                </a>
            </div>
        </form>
