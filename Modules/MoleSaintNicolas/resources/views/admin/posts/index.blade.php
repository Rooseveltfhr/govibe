...
@section('content')
    <div class="mx-auto max-w-5xl px-4 py-10">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-msn-sea-900">Actualités</h1>
            ...
            <table class="min-w-full divide-y divide-msn-sand-200 text-sm">
                ...
                <tbody class="divide-y divide-msn-sand-200">
                    @foreach ($posts as $post)
                        <tr>
                            <td class="px-4 py-3 font-medium text-msn-sea-900">{{ $post->title }}</td>
                            <td class="px-4 py-3 text-msn-sea-700">
                                {{ $post->isPublished() ? 'Publié le '.$post->published_at->format('d/m/Y') : 'Brouillon' }}
                            </td>
                            <td class="px-4 py-3"><x-content-status-badge :status="$post->content_status" /></td>
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('admin.posts.edit', $post) }}" class="text-msn-sea-700 hover:underline">Modifier</a>
                                <form method="POST" action="{{ route('admin.posts.destroy', $post) }}" class="inline" onsubmit="return confirm('Supprimer cet article ?')">
                                    @csrf @method('DELETE')
