...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="flex flex-wrap items-center gap-2">
            <h1 class="text-3xl font-bold text-msn-ink-900 sm:text-4xl">{{ $project->title }}</h1>
        ...
        <p class="mt-6 whitespace-pre-line text-msn-ink-700">{{ $project->description }}</p>

        <hr class="mt-10 border-msn-sand-200">

        <h2 class="mt-8 text-xl font-semibold text-msn-ink-900">
            ...
            @forelse ($project->approvedComments as $comment)
                <div class="rounded-2xl border border-msn-sand-200 bg-white p-4">
                    <div class="flex items-center justify-between">
                        <p class="font-semibold text-msn-ink-900">{{ $comment->author_name }}</p>
                        <p class="text-xs text-msn-ink-700/70">{{ $comment->created_at->format('d/m/Y') }}</p>
                    </div>
                    <p class="mt-2 text-sm text-msn-ink-700">{{ $comment->body }}</p>
                </div>
            @empty
                <p class="text-sm text-msn-ink-700">Aucun avis pour l'instant — soyez le premier à commenter.</p>
            @endforelse
        </div>
        ...
        <form method="POST" action="{{ route('projets.comments.store', $project->slug) }}" class="mt-6 space-y-3">
            ...
            <div>
                <label class="block text-sm font-medium text-msn-ink-900">Votre nom</label>
                <input type="text" name="author_name" required maxlength="100" value="{{ old('author_name') }}"
                       class="mt-1 block w-full rounded-lg border-gray-300 bg-white text-msn-ink-900">
            </div>
            <div>
                <label class="block text-sm font-medium text-msn-ink-900">Votre avis</label>
                <textarea name="body" rows="3" required maxlength="2000"
                          class="mt-1 block w-full rounded-lg border-gray-300 bg-white text-msn-ink-900">{{ old('body') }}</textarea>
            </div>
            <p class="text-xs text-msn-ink-700/70">Votre commentaire sera visible après validation par l'équipe.</p>
