...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="flex flex-wrap items-center gap-2">
            <h1 class="text-3xl font-bold text-msn-sand-100 sm:text-4xl">{{ $project->title }}</h1>
        ...
        <p class="mt-6 whitespace-pre-line text-msn-sand-200">{{ $project->description }}</p>

        <hr class="mt-10 border-msn-sea-700">

        <h2 class="mt-8 text-xl font-semibold text-msn-sand-100">
            Avis ({{ $project->approvedComments->count() }})
        </h2>

        <div class="mt-4 space-y-4">
            @forelse ($project->approvedComments as $comment)
                <div class="rounded-2xl border border-msn-sea-700 bg-msn-sea-900 p-4">
                    <div class="flex items-center justify-between">
                        <p class="font-semibold text-msn-sand-100">{{ $comment->author_name }}</p>
                        <p class="text-xs text-msn-sand-200/70">{{ $comment->created_at->format('d/m/Y') }}</p>
                    </div>
                    <p class="mt-2 text-sm text-msn-sand-200">{{ $comment->body }}</p>
                </div>
            @empty
                <p class="text-sm text-msn-sand-200">Aucun avis pour l'instant — soyez le premier à commenter.</p>
            @endforelse
        </div>
        ...
        <form method="POST" action="{{ route('projets.comments.store', $project->slug) }}" class="mt-6 space-y-3">
            ...
            <div>
                <label class="block text-sm font-medium text-msn-sand-100">Votre nom</label>
                <input type="text" name="author_name" required maxlength="100" value="{{ old('author_name') }}"
                       class="mt-1 block w-full rounded-lg border-gray-300 bg-msn-sea-900 text-msn-sand-100">
            </div>
            <div>
                <label class="block text-sm font-medium text-msn-sand-100">Votre avis</label>
                <textarea name="body" rows="3" required maxlength="2000"
                          class="mt-1 block w-full rounded-lg border-gray-300 bg-msn-sea-900 text-msn-sand-100">{{ old('body') }}</textarea>
            </div>
            <p class="text-xs text-msn-sand-200/70">Votre commentaire sera visible après validation par l'équipe.</p>
