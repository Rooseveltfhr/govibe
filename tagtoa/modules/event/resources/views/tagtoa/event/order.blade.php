{{-- TAGTOA EVENT — confirmation de commande + billets. Variables : $order, $event --}}
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .ok i{font-size:46px;color:var(--green)}
        .ok h1{font-family:var(--fh);font-size:22px;margin-top:10px}
        .ok p{color:#666;font-size:14px;margin-top:4px}
        ...
        .summary{background:#fff;border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:18px}
        .kv{display:flex;justify-content:space-between;padding:7px 0;font-size:14px}.kv span{color:#888}.kv b{font-family:var(--fh)}
        .sec-t{font-family:var(--fh);font-weight:600;font-size:13px;letter-spacing:.05em;text-transform:uppercase;color:#777;margin:8px 0 12px}
...
<div class="wrap">
    <div class="ok">
        ...
        <h1>{{ $order->isPaid() ? __('Billets confirmés!') : __('Commande créée') }}</h1>
        <p>{{ __('Référence') }} : <b>{{ $order->reference }}</b></p>
    </div>

    @if(! $order->isPaid() && $event->payPage)
        <a class="pay" href="{{ url('/pay/'.$event->payPage->alias) }}">
            <i class="fa-solid fa-credit-card"></i> {{ __('Payer') }} {{ number_format($order->total,2) }} {{ $order->currency }}
        </a>
        <p style="text-align:center;font-size:13px;color:#888;margin-top:-8px">{{ __('Après paiement, vos billets seront validés.') }}</p>
    @endif
    ...
    <p class="sec-t">{{ __('Vos billets') }} ({{ $order->tickets->count() }})</p>
    @foreach($order->tickets as $t)
        <a class="tkt" href="{{ route('tagtoa.event.ticket', $t->code) }}">
            <span class="ic"><i class="fa-solid fa-ticket"></i></span>
            <span class="t"><b>{{ optional($t->ticketType)->name }}</b><span>{{ $t->code }}</span></span>
