@extends('erp.layouts.app')
@section('title','Rapport Formations')
@section('page-title','Rapport — Formations & Inscriptions')
@section('page-subtitle','Suivi des formations et participants')

@section('content')
{{-- Filters --}}
<div class="content-card p-4 mb-5">
    <form method="GET" class="flex flex-wrap gap-3 items-end">
        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1">Année</label>
            <select name="year" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                @foreach($availableYears as $y)
                <option value="{{ $y }}" @selected($y==$year)>{{ $y }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1">Statut</label>
            <select name="status" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                <option value="">Tous</option>
                <option value="active" @selected($status==='active')>Actives</option>
                <option value="inactive" @selected($status==='inactive')>Inactives</option>
            </select>
        </div>
        <button type="submit" class="btn-gold px-4 py-2 text-sm">Filtrer</button>
        <a href="{{ route('erp.reports.pdf.formations', ['year'=>$year, 'status'=>$status]) }}"
           class="flex items-center gap-2 bg-red-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-red-700 transition">
            <i class="fas fa-file-pdf"></i> Télécharger PDF
        </a>
        <a href="{{ route('erp.reports.index') }}" class="text-gray-500 text-sm hover:underline self-end pb-2">
            ← Retour aux rapports
        </a>
    </form>
</div>

{{-- KPIs --}}
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    @foreach([
        ['Formations','fa-chalkboard-teacher',$formations->count(),'#d97706'],
        ['Inscriptions totales','fa-users',$totalInscriptions,'#1e3a5f'],
        ['Présents le jour J','fa-user-check',$presents,'#059669'],
        ['Taux présence','fa-percent',$totalInscriptions>0?round($presents/$totalInscriptions*100).'%':'—','#7c3aed'],
    ] as [$label,$icon,$val,$color])
    <div class="content-card p-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background:{{ $color }}15">
            <i class="fas {{ $icon }}" style="color:{{ $color }}"></i>
        </div>
        <div>
            <p class="text-xs text-gray-400">{{ $label }}</p>
            <p class="text-xl font-extrabold text-gray-800">{{ $val }}</p>
        </div>
    </div>
    @endforeach
</div>

{{-- Formations table --}}
<div class="content-card overflow-hidden">
    <div class="p-4 border-b border-gray-100">
        <h3 class="font-semibold text-gray-800">Formations {{ $year }}</h3>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-xs text-gray-500 uppercase">
                <tr>
                    <th class="text-left px-4 py-3">Formation</th>
                    <th class="text-left px-4 py-3">Dates</th>
                    <th class="text-left px-4 py-3">Lieu</th>
                    <th class="text-center px-4 py-3">Max</th>
                    <th class="text-center px-4 py-3">Inscrits</th>
                    <th class="text-center px-4 py-3">Taux remplissage</th>
                    <th class="text-center px-4 py-3">Statut</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                @forelse($formations as $f)
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 font-medium text-gray-900">{{ $f->nom }}</td>
                    <td class="px-4 py-3 text-gray-500">
                        {{ $f->date_debut?->format('d/m/Y') }} → {{ $f->date_fin?->format('d/m/Y') ?? '—' }}
                    </td>
                    <td class="px-4 py-3 text-gray-500">{{ $f->lieu ?? '—' }}</td>
                    <td class="px-4 py-3 text-center text-gray-700">{{ $f->max_participants ?? '∞' }}</td>
                    <td class="px-4 py-3 text-center font-semibold text-gray-900">{{ $f->inscriptions_count }}</td>
                    <td class="px-4 py-3 text-center">
                        @if($f->max_participants)
                        @php $pct = round($f->inscriptions_count / $f->max_participants * 100) @endphp
                        <div class="flex items-center gap-2">
                            <div class="flex-1 bg-gray-100 rounded-full h-1.5">
                                <div class="h-1.5 rounded-full bg-red-500" style="width:{{ min(100,$pct) }}%"></div>
                            </div>
                            <span class="text-xs font-medium text-gray-600 w-10 text-right">{{ $pct }}%</span>
                        </div>
                        @else
                        <span class="text-gray-400 text-xs">—</span>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-center">
                        @if($f->active)
                        <span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-green-100 text-green-700">Active</span>
                        @else
                        <span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-gray-100 text-gray-500">Terminée</span>
                        @endif
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="px-4 py-10 text-center text-gray-400">Aucune formation trouvée.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    </div>
</div>
@endsection
